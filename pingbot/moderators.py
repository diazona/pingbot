...
def update(filename='moderators.json'):
    ...
    with io.open(filename, encoding='UTF-8') as f:
        logger.debug(u'Opened moderator info file {}'.format(filename))
    ...
    logger.debug(u'Loaded mod info: {}'.format(
        ', '.join(
            '{} ({})'.format(site, len(mods)) for site, mods in moderators.iteritems()
        )
    ))
